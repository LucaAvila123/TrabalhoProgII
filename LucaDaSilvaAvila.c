...
void InicializandoTabuleiro(FILE * tabuFinal, int tabuleiro[10][10]);
...
int LinhasIguais_Navios(tNavio navio1, tNavio navio2);
...
int main(int argc, char * argv[]){
    ...
    InicializandoTabuleiro(inicializando, tabuleiro1);
    fprintf(inicializando, "\n\n%s\n", jogador2);
    InicializandoTabuleiro(inicializando, tabuleiro2);
    ...
    resultado = fopen(fraseResultado, "r"); //manipulando um arquivo que o proprio programa gerou
...
void TraduzArquivo_paraNavios(FILE * tabu, tNavio * navios){
    ...
    int i=0;
    while( ! (feof(tabu)) ){
        if(fscanf(tabu, "%[^\n]%*[\n]", escrito) == 1){ //verificando se a linha foi preenchida
            //printf("%s\n", escrito);
...
void TomadaDecisao(char resultante[10][10], char jogador[16], char * decisao){
    char linha='\0'; int coluna=0; char restante='\0';
    int a=0;
    int b=0;
    int i=0, j=0, x=0;
    printf("Jogada de %s:\n\n", jogador);
    while(1){
        ...
        scanf("%s%*[^\n]", decisao);
        sscanf(decisao, "%c%d%c", &linha, &coluna, &restante);

        if((restante != '\0' && restante != '\n')  || linha < 'a' || linha > 'j' || 
            coluna < 1 || coluna > 10) x = 0; //se o comando não está nos conformes
        ...
        if(x == 0){
            ...
            restante = '\0'; linha = '\0'; coluna = 0; //reiniciando as coisas
        }
        else{
            break;
        }
...
void CalculaEstatisticas(tEstatisticas estatisticasA, tEstatisticas estatisticasB, FILE * resultado, FILE * arquivoEstatisticas, char * jogadorA, char * jogadorB){
    ...
    while(1){
        ...
        if(!(strcmp(jogadorA, string))) continue; //para o total nao aumentar de tamanho 
        if(!(strcmp(jogadorB, string))) break; //para que o loop na hora do outro
...
tNavio TraduzLinha_paraNavio(char * dados){
    ...
    sscanf(dados, "%[^;]%*c%d%*c%d%*c%c%d", coisas.tipo, &coisas.identidade, 
        &coisas.orientacao, &coisas.linhas, &coisas.colunas); //o retorno disso eh 5
...
int VerificaTabuleiro(tNavio * navios, int tabuleiro[10][10], int n){
    int i=0, j=0, x=0, estaAli=0, k=0;
    if(n == 0) return 0; //nao vai ter jogo com tabuleiro vazio
    for(x=0; x<n; x++){
        ...
        for(i=0; i<10; i++){
            for(j=0; j<10; j++){
                if(tabuleiro[i][j] == IdentidadeNavio(navios[x])){
                    ...
                    break; //quebra o loop das colunas
                }
            }
            if(estaAli == 1) break; //quebra o loop das linhas
        }
        if(estaAli == 0) return 0;
        for(k = 0; k < x; k++){ //fazendo comparacao entre o navio da vez e todos os que vieram antes
            if(LinhasIguais_Navios(navios[k], navios[x])) return 0; 
        }
    }
    return 1;
}
...
tEstatisticas CalculaDesvioPadrao(tEstatisticas dados, int total, char linhas[total], int colunas[total]){
    ...
    for(i=0; i<total; i++){
        subtracaoLinhas = (float) linhas[i] - 'a' + 1 - dados.mediaLinhas;
        subtracaoLinhas = pow(subtracaoLinhas, 2);
        subtracaoColunas = (float) colunas[i] - dados.mediaColunas;
...
tEstatisticas InicializaEstatisticas(char * jogador){
    ...
    dados.mediaLinhas = 0.0;
    dados.mediaColunas = 0.0;
...
tEstatisticas MediasCalculadas(tEstatisticas dados, float totalLinhas, float totalColunas, int total){
    ...
    dados.mediaLinhas = totalLinhas;
    dados.mediaColunas = totalColunas;
    return dados;
}
tEstatisticas QuantosNaviosForamAtingidos(tEstatisticas dados, int total){
...
void FormaArquivoDeTabuleiro(int argc, char * argv[argc]){
    ...
    if( ! (strcmp(diretorio, "-gt")) ){
        ...
        srand(time(NULL)); //o numero de valores aleatorios gerados pra um unico valor de seed eh suficiente para gerar um tabuleiro valido
        //int x = 0;
        while(1){
            for(i=0; i<5; i++){
